...
def get_census_tracts_data(tract_fips, api_key, dataset, year, variables, max_tracts=1000, clean=False):
    """
    ...
    for state in states_counties_tracts:
        for county in states_counties_tracts[state]:
            ...
            for tracts_chunk in chunks(tracts, max_tracts):
                ...
                print('Downloading {} census vars in {}{} for {} tracts.'.format(len(variables), state,
                                                                                 county, len(tracts_chunk)))

                # get census vars for these tracts and append them to df
                df_tmp = get_tracts_census_vars(api_key=api_key, dataset=dataset,
                                                variables=variables_str, state=state, county=county,
                                                tracts=tracts_str, year=year, clean=clean)
...
def get_tracts_census_vars(api_key, dataset, variables, state, county, tracts, year, clean):
    """
    download a set of census variables for a state + county + tracts
    """
    
    url_template = 'https://api.census.gov/data/{year}/{dataset}/profile?' \
                   'get={variables}&for=tract:{tracts}&key={api_key}&in=state:{state}+county:{county}'
    
    url = url_template.format(api_key=api_key, dataset=dataset, variables=variables,
                              state=state, county=county, tracts=tracts, year=year)
    ...
    try:
        ...
        df['GEOID10'] = df.apply(lambda row: '{}{}{}'.format(row['state'], row['county'], row['tract']), axis='columns')
        df = df.set_index('GEOID10').drop(['state', 'county', 'tract'], axis='columns')
        ...
        return get_tracts_census_vars(api_key, dataset, variables, state, county, tracts, year, clean)
